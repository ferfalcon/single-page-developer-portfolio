...
|---|---|---|---|---|---|
| `Q-001` | What IndexedDB schema, versioning/migration, retention/deletion, form validation, error/success behavior, and privacy requirements apply? | User / product owner | The contact form cannot be treated as complete without these behavioral details. | Requirements/specification and architecture. | Open; not a Stage 0 source-capture blocker. |
...
|---|---|---|---|---|
| Product / Home desktop, tablet, and mobile frames are the intended public page; Components and Design System / Documentation canvases are reference only. | Inferred from node names and hierarchy in `SRC-DS-001`. | Defines the initial source boundary without inventing additional routes. | Stage 1 design audit. | Open |
...
| The post-baseline `AGENTS.md` patch is non-material because it only removes a redundant path statement and leaves project scope and commands unchanged. | Observed repository diff against `SRC-REPO-001`. | No rebaseline is required unless the file changes further or a new material instruction appears. | Before Stage 1 approval and before implementation tasks. | Open for monitoring |

## 4. Architecture Decision

- Separate `ARCHITECTURE.md`: Required by the Full profile when Stage 6 is reached; no Stage 6 decision has been recorded.
- Reason: The user confirmed frontend-only IndexedDB persistence. Its ownership, schema/versioning, lifecycle, privacy, recovery, and validation require explicit architecture handling even without a server API.
...
| 2026-08-14 | Explicit stakeholder decision | None | None | Frontend-only IndexedDB persistence and literal `#` external-link placeholders were specified. | Stage 0 | Start `PROFILE-001` and reconcile Stage 0 narrative before human approval. | In progress |
...
| 2026-08-14 | Standard / Gated | Full / Gated | User confirmed IndexedDB persistence; Full is required by the workflow for persistence and architecture/data-ownership review. | 0 | `PROFILE-001` is in progress and requires human completion. |
...
- **Remaining risks:** `Q-001`, `Q-002`, mutable Figma source, asset mapping, absent font-license text, and the monitored worktree patch.
- **Next permitted action:** A human approver must finish `PROFILE-001` with their real approval identity; after that, they may approve the source/context artifacts, record the Stage 0 gate, and advance. No implementation action is permitted.
